...
function formatCard(card, showDetails = true) {
  if (!card) return null;
  const stats =
    card.atk || card.currentAtk
      ? `${card.currentAtk ?? card.atk}/${card.currentHp ?? card.hp}`
      : '';
...
function buildRulesContext(state, playerIndex) {
  ...
  if (keywords.has('Barrier'))
    rules.push('• Barrier: blocks first damage instance, then removed. Blocks Toxic/Neurotoxic.');
  if (keywords.has('Poisonous'))
    rules.push('• Poisonous: kills attacker when defending. Blocked by Ambush.');
  if (keywords.has('Toxic'))
    rules.push('• Toxic: kills any creature it damages (regardless of HP). Blocked by Barrier.');
  if (keywords.has('Neurotoxic'))
    rules.push(
      '• Neurotoxic: paralyzes enemy after combat (dies end of turn). Blocked by Barrier.'
    );
  if (keywords.has('Hidden'))
    rules.push("• Hidden: can't be attacked but CAN be targeted by spells/abilities.");
  if (keywords.has('Invisible'))
    rules.push("• Invisible: can't be targeted by attacks OR abilities. Mass effects still hit.");
  if (keywords.has('Immune'))
    rules.push('• Immune: only takes damage from direct creature attacks.');
  if (keywords.has('Lure'))
    rules.push(
      '• Lure: MUST target this creature for attacks AND abilities. Overrides Hidden/Invisible.'
    );
...
function formatStatePrompt(qaState, playerIndex, deckName = 'Unknown') {
  ...
  if (qaState.phase === 'Combat') {
    ...
    if (canAttackDirect.length > 0 && totalDirectATK >= oppHP) {
      lines.push(
        `🔥 LETHAL AVAILABLE: You have ${totalDirectATK} direct damage vs rival's ${oppHP} HP. Attack the rival directly to WIN.`
      );
    } else if (canAttackDirect.length > 0) {
      const pctDmg = Math.round((totalDirectATK / oppHP) * 100);
      lines.push(
        `COMBAT MATH: ${canAttackDirect.length} creature(s) can attack rival directly for ${totalDirectATK} damage (${pctDmg}% of rival's ${oppHP} HP). Prioritize face damage unless a trade saves you from lethal.`
      );
    }
    if (canAttackAny.length > 0 && canAttackDirect.length === 0) {
      lines.push(
        `COMBAT MATH: You have ${canAttackAny.length} creature(s) that can attack enemy creatures (but not the rival directly due to summoning sickness).`
      );
    }
    ...
    if (emptyCount === 0) {
      ...
      if (spells.length > 0) {
        lines.push(
          `⚠️ FIELD FULL — but you have ${spells.length} spell(s) in hand. Play a spell to gain advantage!`
        );
      } else if (predators.length > 0 && myCreatures.some((c) => c.type === 'Prey')) {
        lines.push(
          `⚠️ FIELD FULL — consider playing a Predator with EAT to consume a weaker Prey and upgrade your board.`
        );
      } else {
        lines.push(
          `⚠️ FIELD FULL — no spells available. Trade aggressively in combat to open slots for stronger cards next turn.`
        );
      }
    } else if (myCreatures.length === 0) {
      lines.push(
        '⚠️ EMPTY FIELD — play a creature first. Buff/heal spells have no effect without creatures on your field.'
      );
    }
    lines.push('');
  }
  ...
  // Zones
  lines.push(
    `Deck: ${me?.deckSize ?? '?'} cards | Carrion: ${me?.carrion?.length ?? 0} | Exile: ${me?.exile?.length ?? 0}`
  );
  lines.push(
    `Rival: hand ${opp?.handSize ?? '?'} | deck ${opp?.deckSize ?? '?'} | carrion ${opp?.carrion?.length ?? 0}`
  );
  lines.push('');
  ...
  lines.push('');
  lines.push(
    'Think briefly (2-3 sentences max). Then your FINAL LINE must be ONLY the action command.'
  );
