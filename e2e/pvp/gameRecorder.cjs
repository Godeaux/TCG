  ...
  for (const p of ['p1', 'p2']) {
    ...
    if (before[p].carrionSize !== after[p].carrionSize) {
      changes.push({
        type: 'carrion',
        player: p,
        from: before[p].carrionSize,
        to: after[p].carrionSize,
      });
    }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (!bCreature && aCreature) {
        changes.push({
          ...
          player: p,
          slot: i,
          creature: aCreature.name,
          stats: `${aCreature.atk}/${aCreature.hp}`,
        });
        ...
        // Keyword changes
        const addedKw = (aCreature.keywords || []).filter(
          (k) => !(bCreature.keywords || []).includes(k)
        );
        const removedKw = (bCreature.keywords || []).filter(
          (k) => !(aCreature.keywords || []).includes(k)
        );
        if (addedKw.length)
          changes.push({
            type: 'keyword_added',
        ...
        if (bCreature.hasBarrier && !aCreature.hasBarrier)
          changes.push({ type: 'barrier_removed', player: p, slot: i, creature: aCreature.name });
        if (!bCreature.hasBarrier && aCreature.hasBarrier)
          changes.push({ type: 'barrier_added', player: p, slot: i, creature: aCreature.name });
        if (bCreature.isFrozen !== aCreature.isFrozen)
          changes.push({
            type: 'frozen_changed',
            player: p,
            slot: i,
            creature: aCreature.name,
            from: bCreature.isFrozen,
            to: aCreature.isFrozen,
          });
        if (bCreature.isParalyzed !== aCreature.isParalyzed)
          changes.push({
            type: 'paralyzed_changed',
            player: p,
            slot: i,
            creature: aCreature.name,
            from: bCreature.isParalyzed,
            to: aCreature.isParalyzed,
          });
        if (bCreature.isDryDropped !== aCreature.isDryDropped)
          changes.push({
            type: 'drydrop_changed',
            player: p,
            slot: i,
            creature: aCreature.name,
            from: bCreature.isDryDropped,
            to: aCreature.isDryDropped,
          });
      }
    }
  }
