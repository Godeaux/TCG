  ...
  switch (action.type) {
    ...
    default:
      return {
        ...
        error: `Can't execute: ${action.type}`,
      };
  }
}
...
async function executePlay(page, action, state) {
  ...
  if (!card) {
    return {
      success: false,
      description: `No card at index ${handIndex}`,
  ...
  if (targetSlot < 0 && card.type === 'Predator' && !eat?.length && !isDryDrop) {
    return {
      success: false,
      description: 'Field full',
      error: 'No empty slot (specify EAT to make room)',
    };
  }
  ...
  if (isSpell) {
    ...
    if (needsTarget) {
      ...
      if (oppField.length === 0) {
        return {
          success: false,
          description: `Play ${card.name} (${card.type})`,
          error: 'No targets for targeted spell',
        };
      }
      ...
      const targetOptions = oppField.map((c, i) => ({
        index: i,
        name: `${c.name} ${c.currentAtk}/${c.currentHp}${c.keywords?.length ? ' [' + c.keywords.join(',') + ']' : ''}`,
      }));
      ...
      const spellResult = await page.evaluate(
        ({ hi, tSlot }) => {
      ...
      return {
        success: spellResult?.success || false,
        description: desc,
    ...
    if (pending?.type === 'pending_consumption') {
      if (isDryDrop || (!eat && pending.options?.length === 0)) {
        ...
        if (pending.options && pending.options.length > 0 && eat && eat.length > 0) {
          ...
          targetIndices =
            mapped.length > 0 ? mapped : eat.filter((i) => i < pending.options.length);
...
You just played: ${card.name} (${card.type})${card.effectText ? ' — ' + card.effectText : ''}`;
  ...
  for (let chain = 0; chain < 5; chain++) {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      await page.waitForTimeout(400);
      const targeting = await handleTargetingMode(
        page,
        `play:${card.name} (chain ${chain})`,
        gameCtx
      );
      if (targeting.handled) {
        foundUI = true;
        chainsHandled++;
        break;
      }
      const selection = await handleSelectionUI(
        page,
        `play:${card.name} (chain ${chain})`,
        gameCtx
      );
  ...
  if (chainsHandled > 1)
    console.log(`  [CHAIN] Handled ${chainsHandled} chained UIs for ${card.name}`);
...
async function executeActivate(page, action) {
  const result = await page.evaluate(
    ({ idx }) => {
      ...
      const activateBtn = Array.from(buttons).find(
        (b) =>
          b.innerText.toLowerCase().includes('activate') ||
          b.innerText.toLowerCase().includes('yes')
      );
...
async function executePass(page) {
  const result = await page.evaluate(() => {
    ...
    const buttons = overlay.querySelectorAll('button');
    const passBtn = Array.from(buttons).find(
      (b) =>
        b.innerText.toLowerCase().includes('pass') ||
        b.innerText.toLowerCase().includes('no') ||
        b.innerText.toLowerCase().includes('decline')
    );
