 ...
 * - requestSyncFromOpponent: Request sync from opponent when reconnecting
 *
 * Note: loadGameStateFromDatabase is defined in ui.js because it needs
...
export const saveGameStateToDatabase = async (state) => {
  ...
  // Check if game has actually started - don't save empty/initial state
  // This prevents a rejoining player from overwriting saved game with empty state
  const setupComplete = state.setup?.stage === "complete";
  const setupHasRolls = state.setup?.rolls?.some(r => r !== null);
  const gameProgressed = (state.turn ?? 1) > 1;
  const hasCardsInPlay = state.players?.some(p =>
    (p.hand?.length > 0) ||
    (p.field?.some(Boolean)) ||
    (p.carrion?.length > 0)
  );

  if (!setupComplete && !setupHasRolls && !gameProgressed && !hasCardsInPlay) {
    console.log("Skipping save - game has not started yet (turn:", state.turn, "setup:", state.setup?.stage, ")");
    return;
  }
